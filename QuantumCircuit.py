...
class QuantumCircuit:
    ...
    def __init__(self, qubits, cpl_map = None, cpl = None):
        ...
        self.rotating_freq = 2*np.pi * 5.0 # # rotating frame frequency
    ...
    def Rx(self, n, theta):
        ...
        rotating_freq = self.rotating_freq

        # Annihilation operator for the n:th qubit
        a = qt.destroy(n)
        ...
        # Amplitude
        amp = B * (theta % np.pi) / np.pi

        # Master equation simulation
        args = {'amp': amp, 'qscale': 0.032, 'freq': (rotating_freq - drive_freq), 'L': L, 'sigma': sigma}
